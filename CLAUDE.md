...
Homepage = intégration de la maquette **Chez Marcel** (restaurant & glacier, Palavas-les-Flots) : `hero` (ruban), `story` (histoire + chiffres), `highlights` (4 atouts icônes), `menu_list` (carte), `gallery` (aperçu du lieu), `cta_banner` (réservation, tampon), `contact` (lieu + carte + formulaire). Version anglaise réduite sous `/en` (même translationGroup). Autres pages : mentions légales (rich_text, noindex) et `erreur-404`. Actualités : 2 articles publiés + 1 brouillon, catégories « Événements » et « Côté cuisine ». Réglage `analytics_id` = `G-DEMO000000` (barre cookies visible — remplacer ou vider en prod).

Blocs créés pour ce projet (en plus du catalogue du kit) : `story`, `highlights`, `gallery`, `cta_banner`. Le `hero` a un champ « ruban » supplémentaire. Palette (maquette) : sable/encre/argile (terracotta)/laiton, fonts Fraunces + Oswald + Instrument Sans — variables dans `assets/styles/app.css`.
